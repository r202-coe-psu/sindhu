...
class LnMap:
    def __init__(self, center, zoom, min_zoom):
        ...
        """
        CartoDB Positron
        มันคือแผนที่พื้นหลังสี ขาว-เทา (light toned basemap)
        เน้นความเรียบง่าย สว่าง สะอาด ไม่มีสีฉูดฉาด

        เหมาะสำหรับงานที่ต้องการนำ ข้อมูลของผู้ใช้ (markers, heatmap, polygons) มาแสดงทับ เพราะพื้นหลังไม่แย่งจุดสนใจ
        """,
        ...
        # Esri World Topo Map (แผนที่ภูมิประเทศ)
        ...
        # ตั้งชื่อ base_map เพื่อใส่ลง Layer Control
        self.base_maps = {
        ...
        self.map = self.leaflet.map(
            "mapig",
            {
                ...
                "center": self.center,
                "zoom": self.zoom,
                "min_zoom": self.main_zoom,
                "layers": self.openstreet,
                "renderer": self.leaflet.canvas(),
                "scrollWhellZoom": False,  # disable zoom when scroll
        ...
        def on_location_found(e):
            ...
            if hasattr(self, "user_mark") and self.user_mark:
                if not isinstance(self.user_mark, list):
                    ...
                    # ถ้าเป็น list ว่างตาม init ให้สร้าง marker ใหม่
                    self.user_mark = (
                        self.leaflet.marker(
                            self.user_coord,
                            {
    ...
    def on_each_feature(self, feature, layer):
        ...
        def reset_highlight(e):
            for key in self.shapes:
                if self.shapesp[key].hasLayer(layer):
    ...
    def set_shape_with_key(self, data, key):
        ...
        def style(feature):
            ...
            fill_opacity = properties.get("fill_ope", 0.3)
