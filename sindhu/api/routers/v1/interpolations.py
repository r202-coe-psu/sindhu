...
@router.get("/rain")
async def rain(source: str | None = None) -> dict:
    key = f"sindhu:interpolations:rain:{source or 'all'}"
    ...
    result = await services.interpolations.get_rain_interpolation(source)
